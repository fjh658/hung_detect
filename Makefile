...
$(BIN): Package.swift $(SRC) | $(BUILD_DIR)
	...
	@echo 'let buildTime = "'"$$(python3 -c "from datetime import datetime,timezone;import time;now=datetime.now(timezone.utc).astimezone();print(now.strftime('%Y-%m-%dT%H:%M:%S.')+f'{now.microsecond//1000:03d}'+now.strftime('%z')[:3]+':'+now.strftime('%z')[3:])")"'"' >> "$(BUILDINFO_FILE)"
